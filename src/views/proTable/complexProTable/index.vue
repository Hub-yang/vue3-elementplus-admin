...
import type { ResUserList } from '@/api/interface'
...
// 自定义渲染表头（使用tsx语法）
function headerRender(scope: HeaderRenderScope<ResUserList>) {
  return (
...
// 设置列样式
function tableRowClassName({ rowIndex }: { row: ResUserList, rowIndex: number }) {
  if (rowIndex === 2)
    return 'warning-row'
  if (rowIndex === 6)
    return 'success-row'
...
// 单击行
function rowClick(row: ResUserList, column: TableColumnCtx<ResUserList>) {
...
// 删除用户信息
async function deleteAccount(params: ResUserList) {
...
// 重置用户密码
async function resetPass(params: ResUserList) {
